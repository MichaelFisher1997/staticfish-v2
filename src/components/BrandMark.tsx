...
export function BrandMark({ className = "", markClassName = "", showText = false, textClassName = "" }: BrandMarkProps) {
  return (
    <span className={`inline-flex items-center gap-2.5 ${className}`}>
      <span
        className={`relative flex h-9 w-9 shrink-0 items-center justify-center rounded-xl text-white shadow-lg shadow-indigo-500/30 ${markClassName}`}
      ...
      {showText && (
        <span className={`font-display text-xl font-bold tracking-tight text-foreground ${textClassName}`}>
          static<span className="gradient-text">fish</span>
        </span>
      )}
    </span>
  );
